...
function useGameSnapshot() {
  ...
  const remFallbackRef = React.useRef<{ rem: number; at: number }>({ rem: 0, at: 0 });
  ...
  React.useEffect(() => {
    ...
    if (lastIdRef.current !== idBig) {
      ...
      remFallbackRef.current = { rem: 0, at: 0 };
      ...
      if (winPostImm > 0) {
        const immEndGuess = nowS + winPostImm;
        if (immEndGuess > immEndRef.current) {
          immEndRef.current = immEndGuess;
        }
        if (immEndGuess > gloryGuardUntilRef.current) {
          gloryGuardUntilRef.current = immEndGuess;
        }
        if (idBig && idBig !== 0n) {
          try {
            writeMaskUntil(IMMUNITY_KEY, immEndGuess, { messageId: idBig });
          } catch {}
        }
        broadcastImmEnd(immEndGuess);
        showUntilRef.current = Math.max(showUntilRef.current, immEndGuess);
      }
  ...
  React.useEffect(() => {
    const rem = Math.max(0, Number(remChainBN ?? 0n));
    if (rem > 0) {
      remFallbackRef.current = { rem, at: computeChainNow() };
    } else {
      remFallbackRef.current = { rem: 0, at: 0 };
    }
  }, [remChainBN, computeChainNow]);

  const fallbackSnap = remFallbackRef.current;
  const fallbackElapsed =
    fallbackSnap.at > 0 ? Math.max(0, nowSec - fallbackSnap.at) : 0;
  const remFallback =
    fallbackSnap.rem > 0 ? Math.max(0, fallbackSnap.rem - fallbackElapsed) : 0;
  const predictedExposureEnd = startTime && B0secs ? startTime + B0secs : 0;
